...
VARIAVEIS = {
    ...
    "TIPO_ARBOVIROSE": {
        "descricao": "Tipo de arbovirose (agrupamento das classificações)",
        "categorias": {
            "Dengue": "Dengue (todas as formas)",
...
def carregar_dados():
    ...
    if os.path.exists(data_path):
        ...
        # Adiciona coluna de TIPO_ARBOVIROSE (agrupamento de CLASSI_FIN)
        def mapear_tipo_arbovirose(classif):
            if pd.isna(classif):
                return None
            classif = int(classif) if pd.api.types.is_number(classif) else classif
            if classif in [1, 10, 11, 12]:
                return "Dengue"
            elif classif in [2, 4]:
                return "Chikungunya"
            elif classif in [3, 5]:
        ...
        df['TIPO_ARBOVIROSE'] = df['CLASSI_FIN'].apply(mapear_tipo_arbovirose)
...
def pagina_visao_geral():
    ...
    if df is not None:
        ...
        mask_valida = df['NU_ANO'].notna() & (df['CS_SEXO'] != '') & (df['CS_SEXO'] != 'I')
        ...
        with col2:
            tipo_arbovirose_map = VARIAVEIS["TIPO_ARBOVIROSE"]["categorias"]
            tipos_disponiveis = list(tipo_arbovirose_map.keys())
            tipos_nomes = list(tipo_arbovirose_map.values())
            tipos_selecionados_nomes = st.multiselect(
                "Tipo de Arbovirose",
                tipos_nomes,
                default=tipos_nomes
            )
            # Mapear os nomes selecionados de volta para as chaves
            tipos_selecionados = [
                k for k, v in tipo_arbovirose_map.items() 
                if v in tipos_selecionados_nomes
            ]
        ...
        with col4:
            ...
            # Filtrar para só mostrar Masculino e Feminino (remover Ignorado)
            sexo_disponiveis = sorted([s for s in df.loc[mask_valida, 'CS_SEXO'].dropna().unique() if s in ['M', 'F']])
        ...
        # Filtro por tipo de arbovirose (multiselect)
        mask_tipo = df['TIPO_ARBOVIROSE'].isin(tipos_selecionados)
        
        df_filtrado = df[mask_valida & mask_anos & mask_classif & mask_sexo & mask_tipo]
        ...
        # Gráfico de casos por ano (com linhas por tipo)
        # Primeiro, total geral
        df_ano_total = df_filtrado.groupby(df_filtrado['NU_ANO'].astype(int)).size().reset_index(name='Total')
        df_ano_total.columns = ['Ano', 'Total de Casos']
        
        # Agora por tipo de arbovirose
        df_ano_tipo = df_filtrado.groupby(
            [df_filtrado['NU_ANO'].astype(int), 'TIPO_ARBOVIROSE']
        ).size().reset_index(name='Total')
        df_ano_tipo.columns = ['Ano', 'Tipo de Arbovirose', 'Total de Casos']
        
        # Criar figura usando plotly.graph_objects
        fig_ano = go.Figure()
        
        # Adicionar linha geral
        fig_ano.add_trace(
            go.Scatter(
                x=df_ano_total['Ano'], y=df_ano_total['Total de Casos'],
        ...
        # Adicionar linhas por tipo
        for tipo in cores.keys():
            dados_tipo = df_ano_tipo[df_ano_tipo['Tipo de Arbovirose'] == tipo].copy()
            if not dados_tipo.empty:
                # Encontrar o primeiro ano com dados para este tipo
                primeiro_ano = dados_tipo['Ano'].min()
                # Filtrar para começar no primeiro ano (para não mostrar zeros no início)
                dados_tipo = dados_tipo[dados_tipo['Ano'] >= primeiro_ano]
                
                fig_ano.add_trace(
                    go.Scatter(
                        x=dados_tipo['Ano'], y=dados_tipo['Total de Casos'],
                        mode='lines+markers', name=tipo,
                        line=dict(color=cores[tipo], width=2)
                    )
                )
        ...
        # Gráfico de casos por tipo de arbovirose (sempre mostrar)
        df_tipo = df_filtrado['TIPO_ARBOVIROSE'].dropna().value_counts().reset_index()
        df_tipo.columns = ['Tipo de Arbovirose', 'Total de Casos']
        df_tipo['Tipo de Arbovirose'] = df_tipo['Tipo de Arbovirose'].map(
            lambda x: VARIAVEIS["TIPO_ARBOVIROSE"]["categorias"][x] if x in VARIAVEIS["TIPO_ARBOVIROSE"]["categorias"] else x
        )
        
        fig_tipo = px.bar(df_tipo, x='Tipo de Arbovirose', y='Total de Casos', 
                         title='Distribuição por Tipo de Arbovirose',
                         color='Tipo de Arbovirose',
                         color_discrete_map={
                             'Dengue (todas as formas)': '#ff4b5c',
                             'Chikungunya': '#1f77b4',
                             'Zika': '#2ca02c'
                         })
        fig_tipo.update_layout(xaxis_title='Tipo de Arbovirose', showlegend=False)
